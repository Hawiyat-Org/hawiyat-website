...
5. **Proof-band numbers:** may ship verified values `100+ clients` (108 paying), `≈2.6M DZD ARR` (2,621,906 DA, `≈` + `DZD` suffix ALWAYS attached), `10+ resellers`, and `100B+ tokens served`. **AUTHORITATIVE AMENDMENT (2026-08-08, founder):** the colleague's numbers `+100 Clients`, `+10 Resellers`, `+100B Tokens Served` were verified as REAL and must be rendered — they supersede the earlier "+50B is fabrication" note (the dashboard source of truth has since reconciled these figures). **No literal `TODO` string may render on any page** — hide non-verified metrics entirely rather than showing TODO text. **STILL FORBIDDEN: `+50B tokens` as a standalone claim, `+60 clients`, "300 Templates".**
...
- **Main-added components (plan precedence — deleted in Task 8, NOT imported by the rewritten home):** `our-numbers.tsx` (the "Our Numbers" stat block — superseded by the proof band in Step 6; GC5 verified-only policy applies), `ai-playground-dashboard.tsx` (main's hero dashboard — superseded by the Execution Console in Step 2/3). `app/page.tsx` must drop their imports.
- **Kept (restyled here):** `hero-section.tsx`, `ai-playground.tsx` (Execution Console), `pricing.tsx`, `trusted-brands.tsx` (currently main's marquee rewrite — proof band in Step 6 overrides), `faq.tsx`, `call-to-action.tsx`, `newsletter.tsx`, plus `WhatsAppWidget` + `ScrollAnimations` + `Footer` (MUST remain in the rewritten `app/page.tsx` — completeness I6).
...
- [ ] **Step 1: Rewrite `app/page.tsx`** to the lean order above; **KEEP `WhatsAppWidget`, `ScrollAnimations`, `Footer`** (do not drop the primary contact channel); drop Testimonials/commented blocks; drop the old "AI provider built for Algeria" inline block (replaced by Algeria band); remove imports of the six dispositioned components AND the main-added `AiPlaygroundDashboard` + `OurNumbers`.
...
- Delete: `components/bootcamp-effects.tsx`, `registration-modal.tsx`, `floating-elements.tsx`, `services-teaser.tsx` (unused), `testimonials.tsx` (commented out), `styles/globals.css` (dead duplicate), **plus the six home components dispositioned in Task 7**: `benefits-section.tsx`, `prebuilt-tools.tsx`, `additional-features.tsx`, `one-subscription.tsx`, `build-ai-apps.tsx`, `resources.tsx`, **plus the two main-added home components (plan precedence)**: `our-numbers.tsx`, `ai-playground-dashboard.tsx`
...
- [ ] **Step 3: Delete dead files** — `styles/globals.css`, `bootcamp-effects.tsx`, `registration-modal.tsx`, `floating-elements.tsx`, `services-teaser.tsx`, `testimonials.tsx`, the six dispositioned home components (`benefits-section.tsx`, `prebuilt-tools.tsx`, `additional-features.tsx`, `one-subscription.tsx`, `build-ai-apps.tsx`, `resources.tsx`), and the two main-added home components (`our-numbers.tsx`, `ai-playground-dashboard.tsx`). Confirm no imports via `rg` first (Task 7 already removed the home imports). **Do NOT delete `ai-playground.tsx`** (Execution Console, used by hero).
...
> **Scope note (post 8-validator pass, 2026-08-08):** the standard Task 9 steps PLUS the consensus blockers/important items the validators converged on. Validator reports: `validation/v2-*-findings.md`.

**Files:** all changed files.

- [ ] **Step 1: Full verify** — `pnpm lint` (note: 16 pre-existing errors across 6 files at merge-base — FIX them here or document with owner sign-off), `npx tsc --noEmit`, `pnpm build`. `pnpm db:push` runs clean against the Docker Postgres (`postgres:postgres@localhost:5432/hawiyat_db`).
...
- [ ] **Step 3: Copy audit** — run `rg -n "cheap|subscription|credit|2x|5x|20x Claude|Hawiyat Composer(?! AI)|and Claude Code|LLM Credit|50B|60\\+|affordable|unlimited|cheapest|pas cher|10 USD|AI optimization" app components lib/data public` (PCRE via `rg -P`) across ALL 8 services entries + pages + legal + llms.txt. No reseller/cheap-claims; "Hawiyat AI Composer" naming consistent.
- [ ] **Step 4: Proof-band audit** — `rg -rn "50B|60\\+|10 Resellers|300 Templates" app components lib/data public` returns nothing; `100+ clients`, `10+ resellers`, `100B+ tokens served`, `≈2.6M DZD ARR` present on home; **no literal `TODO` string renders on any page**.
- [ ] **Step 5: Design audit (inline checklist)** — verify: (a) no raw hex `bg-[#`/`dark:bg-[#` in components; (b) exactly 2 fonts in `layout.tsx`; (c) lucide only (no `bi-`, no Bootstrap CDN); (d) CTA = `--signal` fill + `--signal-text` in both modes; (e) proof-band numbers policy (GC5); (f) "Hawiyat AI Composer" naming; (g) no refs to `/schedule|/ai-algeria|/cyber-security|/guides|/bootcamp|/templates` in nav/footer/sitemap; (h) `/services/layout.tsx` metadata + JSON-LD use execution naming.
- [ ] **Step 6: Copy-review checklist (marketing I3)** — for every page and every services entry, confirm: (1) no claim without a source; (2) no "cheap/affordable/save money" opener; (3) models appear as *routes*, never the SKU; (4) "is this the execution layer talking?"; (5) `Hawiyat AI Composer` naming consistent; (6) metadata/keywords pruned of cost-reseller terms.

**Consensus blocker/important fixes (folded in — validator-driven):**

- [ ] **Step 7: `/api/chat/route.ts`** (4 agents flagged; completeness/SEO call BLOCKER) — the system prompt still primes Gemini with "2x/5x/20x Claude credits", "cost optimization", "gateway for AI coding tools", "Claude Code/Cursor". **Rewrite the system prompt to execution-layer positioning** (Composer as the AI execution engine: task → plan → route → execute → evaluate → learn; model-agnostic; no credits/cheaper framing), keeping the route functional. If the route is truly orphaned (no UI calls it), note it but keep it working with corrected copy (the dashboard may wire it later).
- [ ] **Step 8: `public/llms.txt`** (3 agents; SEO calls BLOCKER) — stale: old identity + 5 dead routes (`/ai-algeria`, `/hawiyat-composer`, `/guides/claude`, `/cyber-security`). Regenerate: execution-layer identity, current routes only (`/`, `/composer`, `/services`, `/services/[slug]`, `/about`, legal), accurate description.
- [ ] **Step 9: Legal copy** (`app/terms/page.tsx` §5 + `app/privacy/page.tsx`) — remove "AI optimization layer … faster, cheaper", "Starter/Growth/Scale/Team/Coding subscription tiers", "subscription plans, monthly quotas" reseller framing. Rewrite to execution-layer voice; rename to "Hawiyat AI Composer" consistently.
- [ ] **Step 10: `.agents/product-marketing.md`** — stale vs amended GC5: still lists `"10 Resellers"` as FORBIDDEN and omits the now-verified `10+ resellers` / `100B+ tokens`. Update the proof-points table to the 4 verified stats.
- [ ] **Step 11: Layout aliases (SEO)** — `app/layout.tsx` keywords + Organization JSON-LD still contain "Algeria's AI Provider" / "AI provider in Algeria" alternateName/keywords. Replace with execution-layer aliases ("AI infrastructure Algeria", "AI execution layer Algeria").
- [ ] **Step 12: Services SEO keywords (cheap/reseller)** — `lib/data/services.ts` hosting-basic/VIP still open with "cheap/affordable hosting", `hebergement pas cher algerie`. Rewrite those `seo` + `seoContent` + `description` opens to execution-layer value (no cheap/price-first openers).
- [ ] **Step 13: Security hardening (validator-driven):** (a) `/api/subscribe` + `/api/waitlist` add rate limiting / bot protection (reuse the middleware rate-limiter pattern; min interval or honeypot); (b) `lib/email-utils.ts` order templates — HTML-encode `notes`/`name`/`phone` before embedding in emails (injection fix); (c) `middleware.ts` — don't trust `x-forwarded-for` blindly (use a single trustworthy source or note the limitation in a comment); (d) `/api/chat` — if it stays live, note the no-rate-limit exposure (monetary DoS) and either add a per-route limiter or document the decision.
- [ ] **Step 14: Frontend/a11y sweep (validator FAIL domain):** (a) header tokenize: replace `bg-black`/`bg-white`/rounded-lg CTA + gray scales with `--ink`/`--signal` pill per DESIGN §1; (b) services catalog/detail/order: remove purple gradients (`from-violet-500 to-purple-600`), `bg-white/40` cards → tokens; (c) `/about`: tokenize remaining `bg-black`/`bg-white` CTAs + 7xl/8xl `font-light` headings → Space Grotesk scale; (d) reduced-motion: gate `.reveal-up` + `#dashboard` GSAP behind `prefers-reduced-motion` in `scroll-animations.tsx` (currently un-gated); (e) skip-link: target a wrapper ABOVE the header so it actually skips nav (currently targets `#content` which contains the header); (f) FAQ button: restore a focus ring (remove bare `focus:outline-none` or add replacement); (g) mobile menu: add `aria-expanded` + body scroll-lock; (h) `/services` + `/services/[slug]`: nested `<main>` landmarks → use `<div>`/`<section>` for the inner wrapper; (i) services pages: replace `text-muted-foreground`/`bg-background` with `text-muted-ink`/tokens.
- [ ] **Step 15: Commit** — `git commit -m "fix(qa): final rebrand QA — chat/llms.txt/legal copy, product-marketing update, SEO aliases, security hardening, frontend+a11y sweep"`
