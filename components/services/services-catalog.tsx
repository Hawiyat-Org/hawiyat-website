    ...
    >
      ...
      <div
        className={`relative h-full w-full transition-all duration-500 [transform-style:preserve-3d] ${
          !isMobile ? "group-hover:min-h-[600px] group-hover:[transform:rotateY(180deg)]" : ""
        } ${
          isFlipped ? "min-h-[600px] [transform:rotateY(180deg)]" : "min-h-[480px]"
        }`}
      >
         {/* ── FRONT FACE ── */}
          ...
          <div className="relative h-full rounded-2xl border border-border/60 bg-card dark:bg-secondary backdrop-blur-xl overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="relative flex flex-col flex-1 p-6 text-foreground">
              ...
              <div className="mb-4 shrink-0">
                ...
                <p className="mt-2 text-xs text-muted-foreground"><strong className="text-foreground">Useful for:</strong> {service.useCases}</p>
              </div>

              <ul className="mb-4 space-y-1 text-xs text-muted-foreground">
                {service.features.slice(0, 4).map((feature) => (
                  <li key={feature} className="flex gap-2"><CheckCircle className="mt-0.5 h-3.5 w-3.5 shrink-0" />{feature}</li>
                ))}
              </ul>
